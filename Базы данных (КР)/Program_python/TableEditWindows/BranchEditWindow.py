        ...
        try:
            ...
            for r in range(len(self.rows)):
                ...
                if not the_same:
                    ...
                    cur.execute(f"""UPDATE branch SET branch_name='{data[r][1]}',
                                supervisor='{data[r][2]}' WHERE id_branch={data[r][0]}""")
                    self.conn.commit()
                    cur.close()
            if len(self.rows) < len(data):
                for r in range(len(self.rows), len(data)):
                    cur = self.conn.cursor()
                    cur.execute(f"""INSERT INTO branch (branch_name, supervisor)
