        ...
        try:
            ...
            for r in range(len(self.rows)):
                ...
                if not the_same:
                    ...
                    cur.execute(f"""UPDATE technique SET tec_name='{data[r][1]}', 
                                    id_department={data[r][2]}, id_tec_type={data[r][2]} 
            ...
            if len(self.rows) < len(data):
                for r in range(len(self.rows), len(data)):
                    cur = self.conn.cursor()
                    cur.execute(f"""INSERT INTO technique (tec_name, id_department, id_tec_type)
