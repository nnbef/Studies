        ...
        try:
            ...
            for r in range(len(self.rows)):
                ...
                if not the_same:
                    ...
                    cur.execute(f"""UPDATE department SET dep_name='{data[r][1]}',
                                    id_branch={data[r][2]} WHERE id_department={data[r][0]}""")
                    self.conn.commit()
                    cur.close()
            if len(self.rows) < len(data):
                for r in range(len(self.rows), len(data)):
                    cur = self.conn.cursor()
                    cur.execute(f"""INSERT INTO department (dep_name, id_branch)
