        ...
        try:
            ...
            for r in range(len(self.rows)):
                ...
                if not the_same:
                    ...
                    cur.execute(f"""UPDATE repair_type SET rep_type_name='{data[r][1]}',
                                    standart_time='{data[r][2]}' WHERE id_rep_type={data[r][0]}""")
                    self.conn.commit()
                    cur.close()
            if len(self.rows) < len(data):
                for r in range(len(self.rows), len(data)):
                    cur = self.conn.cursor()
                    cur.execute(f"""INSERT INTO repair_type (rep_type_name, standart_time)
